...
<form action="#" method="post" class="row ">
    ...
    <div class="col-sm-12">
        ...
        <div class="pull-right">
            ...
            <a href="{{url('admin/area_office')}}" type="button" class="btn btn-lg btn-primary">Cancel</a>
            ...
            @endif
            @if(!$branch)
                <h3>Something went wrong head  <a class="btn btn-danger btn-lg" href="{{url('/admin/area_office/errore')}}">Back</a></h3>
            @endif
        </div>
    </div>
</div>
            ...
            buttons: {
                confirm: function () {
                    $.ajax({
                        ...
                        error: function() {
                            window.location = "{{url('/admin/area_office/errord')}}";
                        },
                        dataType: 'text',
                        success: function() {
                            window.location = "{{url('/admin/area_office/deleted')}}";
            ...
            $.confirm({
                ...
                buttons: {
                    confirm: function () {
                        $.ajax({
                            type: 'POST',
                            url: "{{url('/admin/update_areaoffice')}}",
                            data: {
                                id:id,
                                name:name,
                                email:email,
                            ...
                            error: function() {
                                window.location = "{{url('/admin/area_office/erroru')}}";
                            },
                            dataType: 'text',
                            success: function() {
                                window.location = "{{url('/admin/area_office/update')}}";
