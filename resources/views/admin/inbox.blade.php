...
@section('content')
        ...
        jQuery( document ).ready( function( $ ) {
            ...
            $table1.closest( '.dataTables_wrapper' ).find( 'select' ).select2( {
                minimumResultsForSearch: -1
                    ...
                    @endif
                    @if($id=='add')
                        <div class="alert alert-success"> New Message have been Added Succesfully <a class='close' data-dismiss='alert'>&times;</a></div>
                    @endif
                    @if($id=='deleted')
                        <div class="alert alert-success"> New Message Deleted Succesfully <a class='close' data-dismiss='alert'>&times;</a></div>
                    @endif
                    @if($id=='errord')
        ...
        <table class="table table-striped table-bordered" id="table-1">
            ...
            @foreach($inbox as $message)
                <tr>
                    ...
                    <td>{{$message->aname}}</td>
                    <td>
                        <a href="{{url('/admin/edit_inbox')}}{{'/'.$message->id}}" class="btn btn-primary btn-sm done"> Update<span class="glyphicon glyphicon-edit"></span></a>
                    </td>
                </tr>
            @endforeach
        ...
        <div>
             <td colspan="4"><div class="text-center"><a href="{{url('/admin/add_inbox')}}" type="button" class="btn btn-sm btn-success"><span class="glyphicon glyphicon-plus"></span> New Message </a></div></td>
        </div>
