...
@section('content')
        ...
        jQuery( document ).ready( function( $ ) {
            ...
            $table1.closest( '.dataTables_wrapper' ).find( 'select' ).select2( {
                minimumResultsForSearch: -1
                    ...
                    @endif
                    @if($id=='update')
                        <div class="alert alert-success"> Head Office have been Updated Succesfully <a class='close' data-dismiss='alert'>&times;</a></div>
                    @endif
                    @if($id=='erroru')
        ...
        <table class="table table-striped table-bordered" id="table-1">
            ...
            @foreach($branch as $branches)
                <tr>
                    ...
                    <td>
                        <a href="{{url('/admin/editheadoffice')}}{{'/'.$branches->id}}" class="btn btn-primary btn-sm done"> Update<span class="glyphicon glyphicon-edit"></span></a>
                    </td>
                </tr>
            @endforeach
        ...
        <div>
             <td colspan="4"><div class="text-center"><a href="{{url('/admin/add_headoffice')}}" type="button" class="btn btn-sm btn-success"><span class="glyphicon glyphicon-plus"></span> Add Head Office</a></div></td>
        </div>
