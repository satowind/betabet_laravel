...
@section('content')
        ...
        jQuery( document ).ready( function( $ ) {
            ...
            $table1.closest( '.dataTables_wrapper' ).find( 'select' ).select2( {
                minimumResultsForSearch: -1
                    ...
                    @endif
                    @if($id=='update')
                        <div class="alert alert-success"> Branch Office have been Updated Succesfully <a class='close' data-dismiss='alert'>&times;</a></div>
                    @endif
                    @if($id=='erroru')
                    ...
                    @endif
                    @if($id=='deleted')
                        <div class="alert alert-success"> Branch Office Deleted Succesfully <a class='close' data-dismiss='alert'>&times;</a></div>
                    @endif
                    @if($id=='errord')
        ...
        <table class="table table-striped table-bordered" id="table-1">
            ...
            @foreach($branch as $branches)
                <tr>
                    <td>{{$branches->name}}</td>
                    <td>{{$branches->bet9ja_id}}</td>
                    <td>{{$branches->bet9ja_code}}</td>
                    ...
                    <td>
                        <a href="{{url('/admin/editbranchoffice')}}{{'/'.$branches->id}}" class="btn btn-primary btn-sm done"> Update<span class="glyphicon glyphicon-edit"></span></a>
                    </td>
                </tr>
            @endforeach
        ...
        <div>
             <td colspan="4"><div class="text-center"><a href="{{url('admin/add_branchoffice')}}" type="button" class="btn btn-sm btn-success"><span class="glyphicon glyphicon-plus"></span> Add Post</a></div></td>
        </div>
