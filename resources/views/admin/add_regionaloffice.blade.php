...
<form action="" method="post" class="row">
   ...
   <label for="client-email"><h3 class="h3 text-danger">Local Government</h3></label> 
    ...
    <div class="col-sm-12">
        <br><br>
            <a href="{{url('admin/regional_office')}}"  class="btn btn-lg btn-default">Cancel</a>
            <button type="button" onclick="add_regionaloffice()" class="btn btn-lg btn-success pull-right" name="addFAQ">Add</button>
    </div>
</form>
</div></div></div>
            ...
            $.confirm({
                ...
                buttons: {
                    confirm: function () {
                        $.ajax({
                            type: 'POST',
                            url: "{{url('/admin/add_regionaloffice')}}",
                            data: {
                            ...
                            error: function() {
                                window.location = "{{url('/admin/regional_office/errora')}}";
                            },
                            dataType: 'text',
                            success: function() {
                                window.location = "{{url('/admin/regional_office/add')}}";
